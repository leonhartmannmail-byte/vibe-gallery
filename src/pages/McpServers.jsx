...
function McpServers() {
  ...
  const { items, total, loading, loadingMore, hasMore, loadMore, categories } = usePaginatedList('mcp_servers', {
    search: debouncedSearch,
    category: activeCategory,
    transform: transformMcp,
  })
    ...
    <div className="listing-page">
      ...
      <div className="listing-container">
        <motion.div className="listing-header" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <Server size={28} className="listing-icon listing-icon--mcp" />
          <div>
            <h1 className="listing-title">MCP 服务</h1>
            <p className="listing-subtitle">Model Context Protocol 工具集，共 {total} 个</p>
          </div>
        </motion.div>
        ...
        <motion.div className="listing-toolbar" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.1 }}>
          <div className="listing-search-wrap">
            <Search size={16} />
            <input className="listing-search-input" placeholder="搜索服务名称..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        ...
        {loading ? (
          ...
          <div className="listing-empty">没有找到相关服务</div>
        ) : (
          <>
