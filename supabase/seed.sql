...
('frontend-design', '打造独具辨识度的生产级前端界面，强调大胆美学选择与精细实现，涵盖网页组件、落地页、仪表盘等各类UI构建需求。', '软件开发', 1),
...
('java-dev', '提供 Java 项目开发全流程的标准化指导，涵盖代码风格、异常与空值处理、并发编程、单元测试。', '软件开发', 7),
('vue', '提供 Vue 3 单文件组件开发、组合式函数编写、客户端工具函数设计及单元测试的完整实践指南。', '软件开发', 8),
('skill-creator', '指导创建高效技能模块，通过封装专用工作流、工具集成与领域知识，将通用模型转化为特定任务专家。', '效率工具', 9),
('codex-skill', '通过自动化代码执行能力，实现从需求分析到代码修改的端到端编程任务处理。', '效率工具', 10),
('claude-skill', '提供无需人工干预的自动化代码处理能力，支持代码分析、缺陷修复、功能实现、安全审查等任务。', '效率工具', 11),
('mcp-builder', '指导构建高质量的 MCP 服务器，通过合理设计工具使 LLM 能高效调用外部服务。', '开发工具', 12),
('frontend-code-review', '执行前端代码审查，覆盖 React 与 Next.js 项目的类型安全、组件结构及性能优化。', '测试与安全', 13),
('frontend-testing', '提供前端单元与组件测试能力，支持对逻辑函数、用户交互及界面渲染的验证。', '测试与安全', 14),
('doc-coauthoring', '协助用户系统化地协同撰写各类结构化文档，涵盖技术规范、决策说明、产品提案等场景。', '文档处理', 15),
('prompt-lookup', '专注于AI提示词的发现、获取与优化，支持按关键词、类型、领域和标签检索高质量提示模板。', '效率工具', 16),
('skill-lookup', '支持在技能生态中发现、获取和安装可复用的 AI 能力组件。', '效率工具', 17),
